...
void main() async {
  ...
  try {
    ...
    final timetablesData = await timetablesResponse
    ...
    if (timetablesResponse.statusCode == 200) {
      ...
      for (var timetable in timetables) {
        print(
          '   - Semester: ${timetable['semester']}, Course: ${timetable['course_code']}, Batch: ${timetable['batch']}, Year: ${timetable['academic_year']}',
        );
      }
    } else {
    ...
    if (periodsResponse.statusCode == 200) {
      ...
      for (var period in periods) {
        print(
          '   - ${period['day']}: ${period['start_time']}-${period['end_time']} | ${period['subject_name']}',
        );
      }
    } else {
    ...
    final specificRequest = await client.getUrl(
      Uri.parse(
        '$supabaseUrl/rest/v1/timetables?semester=eq.5&course_code=eq.CSE&batch=eq.1&academic_year=eq.2025',
      ),
    );
